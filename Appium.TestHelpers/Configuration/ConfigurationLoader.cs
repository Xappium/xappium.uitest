...
namespace Appium.TestHelpers.Configuration
{
    internal static class ConfigurationLoader
    {
        ...
        public static UITestConfiguration Load(Assembly assembly)
        {
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
                throw new PlatformNotSupportedException("UI Tests should be run on macOS only.");

            var config = new UITestConfiguration();

            if (File.Exists(Constants.UITestConfigFile))
            {
                config = JsonConvert.DeserializeObject<UITestConfiguration>(File.ReadAllText(Constants.UITestConfigFile));
            }

            var resourceId = assembly.GetManifestResourceNames().FirstOrDefault(x => x.EndsWith(Constants.UITestConfigFile));
